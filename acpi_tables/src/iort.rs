...
pub struct MemoryAccessProperties {
    /// The Root Complex is fully cache coherent.
    pub cache_coherent: bool,
    /// A combination of values from [`allocation_hints`].
    pub allocation_hints: u8,
    /// A combination of values from [`memory_access_flags`].
    pub flags: u8,
}
...
pub struct Config<'a> {
    /// GIC ITS identifiers referenced by the ITS Group node.
    pub its_ids: &'a [u32],
    /// PCI segment number.
    pub pci_segment: u32,
    /// Memory access properties of the PCI Root Complex.
    pub memory_access_properties: MemoryAccessProperties,
    /// PCI Root Complex ATS attributes from [`ats_attributes`].
    pub ats_attributes: u32,
    /// Memory address limit in bits.
    pub memory_address_limit: u8,
    /// Maximum supported PASID width, in the range 0 through 20.
    pub pasid_capabilities: u16,
    /// PCI Root Complex node flags from [`root_complex_flags`].
    pub root_complex_flags: u32,
    /// Number of PCI Request IDs mapped to the ITS Group.
    pub id_count: u32,
}
